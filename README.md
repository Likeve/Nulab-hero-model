...
- **HDRI**（默认 `parking_garage_1k.hdr`，仓库内为 **512×256** Radiance，由 1k 等距图压缩以减轻加载；原图备份为 `parking_garage_1k_1024_backup.hdr` 勿提交）经 PMREM 生成环境；支持环境旋转与循环扫光动画。
- **滚轮 / 触屏入场：** 模型就绪后相机先置于**远景**；桌面为首次**向下滚动**；**平板（≤810px）与手机（≤390px）**为手指**上移滑动手势**（与向下滚意图一致）触发同一套入场 + zoom。样式按 810 / 390 断点收紧加载条与文案。**dolly 回默认机位**等与桌面相同；入场与 zoom 中不拦截滚轮；仅桌面首次滚轮触发时 `preventDefault`。
